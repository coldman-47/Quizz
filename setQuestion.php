...
$enonce = $questionnaires['question']; $type = $questionnaires['type']; $reponse = $questionnaires['reponses']; $score = $questionnaires['score']; $vraies = $questionnaires['vrai'];
$error = '';$classError = '';
$num = $_GET['num']; $exit = '';
const ERROR = 'error';
            ...
            if(sizeof($vraies) > 0){
                if($type == 'qcm' || (in_array($type,['radio','text']) && sizeof($vraies) == 1)){
                    if($type == 'qcm' && (sizeof($reponses) == sizeof($vraies))){
                        ...
                        $questionnaires['question'][$num] = $enonce;
                        $questionnaires['score'][$num] = (int)$score;
                        $questionnaires['type'][$num] = $type;
                        $questionnaires['reponses'][$num] = $reponses;
                        $questionnaires['vrai'][$num] = $vraies;
                        file_put_contents('questions.json', json_encode($questionnaires,JSON_PRETTY_PRINT));
                        $exit = 'alert("Question modifé!"); window.close();';
                    }
                }else{
...
<body>
<div id="questions">
    <h2 style="color:deepskyblue; margin-top:-0.5em; text-transform:uppercase">Modification de la question numéro <?= $num+1 ?></h2>
    <form id="formQst" method="post">
        <div>
            <label for="">Questions</label>
            <textarea name="enonce" class="requis"><?= $enonce[$num] ?></textarea>
        </div>
        <div>
            <label for="">Nombre de points</label>
            <input value="<?= $score[$num] ?>" min="1" id="pts" name="points" type="number" class="requis">
        </div>
        <div>
        ...
        <div id="reponses">
        <?php
                if(in_array($type[$num],['qcm','radio'])){
                    for($i = 0; $i<sizeof($reponse[$num]); $i++){
                        ?>
                        <div class="rep">
                            <label for="">Reponse <?= $i+1 ?></label>
                            <input class="requis" name="rep-<?= $i+1 ?>" value="<?= $reponse[$num][$i] ?>">
                            <input <?php if(in_array($reponse[$num][$i],$vraies[$num])) echo "checked" ?> type="checkbox" name="cb<?= $i+1 ?>" id="">
                            <input onClick="trash(<?= $i+1 ?>)" type="button" class="del" id="del<?= $i+1 ?>" value="🗑">
                        </div>
                        <?php
                    }
                }else{
                    ...
                    <div id="rep-0">
                        ...
                        <input class="requis" name="rep-0" placeholder="Saisir la bonne réponse">
                        <input type="checkbox" name="cb0" checked hidden>
                    </div>
                    <?php
                }
            ?>
...
<script>
<?= $exit ?>
let options = document.getElementsByTagName('option'), i;
for(i = 0; i<options.length; i++){
    if(options[i].value == "<?= $type[$num] ?>"){
